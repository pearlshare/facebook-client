...
module.exports = function facebookClient(config) {
  ...
  function client(authToken) {
     ...
     * @returns {Promise} a promise providing the URL of the users profile image
     */
    function profilePhotoRedirectUrl(userId) {

      if (userId && userId !== "") {
        return Bluebird.resolve(buildUrl(`${userId}/picture`));
      } else {
        return me()
          .then(function (res) {
            var id = get(res, "body.data.id");
            if (id && id !== "") {
              return buildUrl(`${res.body.data.id}/picture`);
            } else {
              return null;
            }
          })
    ...
    return {
      ...
      profilePhotoRedirectUrl: profilePhotoRedirectUrl
    };
  }
